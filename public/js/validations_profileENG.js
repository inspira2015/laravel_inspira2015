...
var errores = "";
		...
		{
			errores += "Use only one name that contains only letters.<br />";
		}
			
		//Verificar si los apellidos no tienen numeros o signos
		if(!name_validation(apellido))
		{
			errores += "Use only one last name that contains only letters..<br />";
		}
			
		//Verificar si el correo es válido
		if(!email_validation(email))
		{
			errores += "The email provided is not valid.<br />";
		}
			
		//Verificar contraseñas son iguales
		if(!conpassword_validation(password, password_check))
		{
			errores += "Passwords must be the same.<br />";
			//alert("FALSO!");
		}
		...
		{
			errores += "The cell is invalid.<br />";
		}
		else 
		{
			phone_inserted = true;	
		...
		//Verificar si hay al menos un telefono
		if(! phone_inserted)
		{
			errores += "You must capture at least one phone.<br />";
		}
		
		if(!username_validation(pais))
		{
			errores += "Select a country.<br />";
		}

		//Verificar si los estados 
		if(!state_validation(estado))
		{
			errores += "Select a state.<br />";
		}
