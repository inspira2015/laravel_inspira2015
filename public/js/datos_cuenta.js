...
function getData2()
{
	...
	success     : function(response)
	{
		...
		$('#contrasena').html('Contraseña: <input type="password" style="background-color:#f6f7f7 !important; border:none;" disabled value="'+(response.password)+'"/>');
  }});
}
...
{
	var campos2 = $('#campos2');
	$('#cambiar2').html('<img src="images/guardar.png"/>');
	$(campos2).html('');
	...
	$('<label class="controleslabel">Contraseña: </label>').appendTo(campos2);
	...
	$('<label class="controleslabel">Confirmar: </label>').appendTo(campos2);
...
function updateData2()
{
	...
	else
	{
		...
		$('<br>').appendTo(campos2);
		$('<br>').appendTo(campos2);
		$('<p class="controleslabel" >Las contraseñas deben ser iguales</p>').appendTo(campos2);
		edicion2 = true;
	}
}
...
{
	var campos2 = $('#campos2');
	$('#cambiar2').html('<img src="images/cambiar.png"/>');
	$(campos2).html('');

	$('<p id = "correo" class=""> </p>').html('Email: '+correo).appendTo(campos2);
	$('<p id = "contrasena" class=""> </p>').html('Contraseña: '+contrasena).appendTo(campos2);
